...
포트를 두는 이유를 "나중에 PG사를 바꿀 수 있으니까"로 설명하는 경우가 많다. 하지만 대부분의
서비스는 PG사를 한 번도 바꾸지 않는다. 그런데도 포트가 필요한 이유는 따로 있다.

**오염을 막기 위해서다.**

전문번호 형식, `VTIM` 같은 응답코드, 3000 전문(타행이체불능 통지) — 이건 전부 핵토파이낸셜의
사정이다. 이게 도메인으로 들어오면 `EwaTransfer`가 특정 은행의 프로토콜을 알게 된다.

지금은 어댑터가 그것을 세 가지로 번역해서 넘긴다.
...
도메인은 성공·미확정·실패만 안다. 은행이 어떤 코드로 답하는지는 모른다.

목 어댑터와 실제 어댑터가 공존하는 건 **그 경계가 실제로 서 있다는 증거**지, 포트를 둔
목적이 아니다. 교체 가능성은 경계를 제대로 그은 결과로 따라오는 것이다.

그리고 이 글에서 다룬 버그는 **그 경계가 반대 방향으로 새고 있었다는 이야기**다. 포트가
`Worker` 엔티티를 요구했으니, 인프라가 도메인의 영속성 상태를 알고 있었던 셈이다.

> **포트는 교체를 위해서가 아니라 오염을 막기 위해 있다.**

### 그럼 리포지토리도 포트로 빼야 하지 않나
...
**은행 프로토콜과 JPA는 오염의 성격이 다르다.** 전자는 도메인이 몰라도 되는 남의 사정이지만,
JPA는 이미 도메인 모델링 방식 자체에 녹아 있다. 걷어내려면 모델을 두 벌로 나눠야 하고,
그 대가가 이 규모에서는 크다.

그래서 정확히 말하면 **외부 연동에만 포트/어댑터를 적용**했다. 전면 헥사고날이 아니다.
...
이 프로젝트의 `domain` 패키지는 DDD의 도메인 레이어가 아니라 **업무 단위 묶음**이다.
`domain/payperiod` 안에 엔티티·서비스·컨트롤러가 함께 있다. 계층이 아니라 도메인 기준으로
나눈 것이고, `domain/port`만 헥사고날 의미의 포트다.
