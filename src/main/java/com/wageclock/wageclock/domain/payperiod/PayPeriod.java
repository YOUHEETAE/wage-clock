...
public class PayPeriod extends BaseEntity {
    ...
    public enum PayPeriodStatus {
        ACTIVE,
        // 정산 시작과 마감 사이. 사장 입금을 기다리는 구간이라 수 분~수 시간 열려 있는데,
        // 그동안 금액이 움직이면 이미 확정된 이체액(BulkSettlementItem.amount)과 어긋난다.
        // 출근을 막아 금액을 고정하고, 정산이 무산되면 ACTIVE로 되돌린다.
        SETTLING,
        CLOSED
    }
    ...
    public void startSettling() {
        if(!(this.status == PayPeriodStatus.ACTIVE)) {
            throw new IllegalStateException("PayPeriodStatus is not ACTIVE");
        }
        this.status = PayPeriodStatus.SETTLING;
    }

    public void reopen() {
        if(!(this.status == PayPeriodStatus.SETTLING)) {
            throw new IllegalStateException("PayPeriodStatus is not SETTLING");
        }
        this.status = PayPeriodStatus.ACTIVE;
    }
