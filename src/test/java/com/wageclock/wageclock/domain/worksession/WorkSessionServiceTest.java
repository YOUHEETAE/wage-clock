...
public class WorkSessionServiceTest {
    ...
    void clockIn_employment_없음_예외() {
        ...
        assertThrows(NotFoundException.class,
                () -> workSessionService.clockIn(new ClockInRequest(1L)));
    }
    ...
    void clockIn_정상_응답_반환() {
        ...
        ClockInResponse response = workSessionService.clockIn(new ClockInRequest(1L));
    ...
    void clockOut_세션_없음_예외() {
        ...
        assertThrows(NotFoundException.class,
                () -> workSessionService.clockOut(new ClockOutRequest(1L)));
    }

    @Test
    void clockOut_다른_워커_접근_예외() {
        when(workSessionRepository.findById(1L)).thenReturn(Optional.of(savedWorkSession));
        when(savedWorkSession.getEmployment()).thenReturn(employment);
        when(employment.getWorker()).thenReturn(worker);
        when(worker.getId()).thenReturn(2L);
        assertThrows(UnauthorizedException.class,
                () -> workSessionService.clockOut(new ClockOutRequest(1L)));
    ...
    void clockOut_정상_응답_반환() {
        ...
        ClockOutResponse response = workSessionService.clockOut(new ClockOutRequest(1L));
