...
public class PortOneWebhookControllerTest {
    ...
    void webhook_PAID_수신_성공() throws Exception {
        ...
        verify(bulkSettlementService).initiateBulkSettlement("test-payment-id");
    }
    ...
    void webhook_CANCELLED_수신_성공() throws Exception {
        ...
        mockMvc.perform(post("/webhook")
                        ...
                        .content(payload))
                .andExpect(status().isOk());
        verify(bulkSettlementService).failedPayment("test-payment-id");
    }
    ...
    void 알_수_없는_타입_무시() throws Exception {
        ...
        mockMvc.perform(post("/webhook")
                ...
                .content(payload))
                .andExpect(status().isOk());
        verifyNoInteractions(bulkSettlementService);
    }
}
